...
$slct_stmnt = 'SELECT * FROM luxyart_tb_sitemap WHERE sitemap_id in (20) and status = 1 ORDER BY FIELD(sitemap_id,20)';
    ...
    while($rowMain = $resultMain->fetch_assoc()) {
		...
		if($is_params == 0){
			...
			if($sitemap_id == 20){
				...
				$sql = "SELECT cate_id, product_category_id FROM luxyart_tb_product_category WHERE status_cate = 1 ORDER BY cate_id ASC";
				...
				if($result->num_rows > 0){
					...
					while($row = $result->fetch_assoc()){
						...
						$link1 = str_replace("{category_id}", $product_category_id, $link);
...
$output = $xml->saveXML();
$xml->save("category.xml");
$con->close();

echo "Create category.xml ok!";
