...
class CI_Cart {
	...
	function update($items = array())
	{
		...
		$save_cart = FALSE;
		if (isset($items['id']) AND isset($items['qty']))
		{
		...
		else
		{
			foreach ($items as $val)
			{
				if (is_array($val) AND isset($val['id']) AND isset($val['qty']))
				{
	...
	function _update($items = array())
	{
		...
		if ( ! isset($items['qty']) OR ! isset($items['id']) OR ! isset($this->_cart_contents[$items['id']]))
		{
			return FALSE;
		}
		...
		if ($this->_cart_contents[$items['id']]['qty'] == $items['qty'])
		{
			return FALSE;
		}
		...
		{
			$this->_cart_contents[$items['id']]['qty'] = $items['qty'];
		}
