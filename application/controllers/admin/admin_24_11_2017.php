...
class Admin extends MY_Admin {
    ...
    public function messageUpdate($data) {
        if($data){
            $this->res(true, "Update success !");
        }else{
            $this->res(false, "Update error !");
        }
        return $this->res();
    }
    ...
    public function messageCreate($data) {
        if($data){
            $this->res(true, "Insert success !");
        }else{
            $this->res(false, "Insert error !");
        }
        return $this->res();
    }
    ...
    public function messageDelete($data) {
        if($data){
            $this->res(true, "Delete success !");
        }else{
            $this->res(false, "Delete error !");
        }
        return $this->res();
    }
    ...
    public function upload($key, $name, $folder){
    ...
    public function page_statistic() {
        ...
        for($i = 1; $i <= 12; $i++) {
            $month = $i<10?'0'.$i:$i;
            if($month == date('m')) break;
            $this->db->from('luxyart_tb_management_point');
            $this->db->where('status_change_point',2);
            $this->db->where('date_add >=', date('Y').'-'.$month.'-00 00:00:00');
            $this->db->where('date_add <', date('Y').'-'.($month+1).'-00 00:00:00');
            $this->db->select('SUM(point) as total_point');
            $result = $this->db->get()->row();
            array_push($use_point_month, $result->total_point==''?0:$result->total_point);
        }
        $data['use_point_month'] = "[".implode($use_point_month, ', ')."]";
        ...
        for($i = 1; $i <= 12; $i++) {
            ...
            $this->db->select('SUM(point) as total_point');
            $result = $this->db->get()->row();
            array_push($buy_point_month, $result->total_point==''?0:$result->total_point);
        }
        $data['buy_point_month'] = "[".implode($buy_point_month, ', ')."]";

        $use_point_week = [];
        for($i = 1; $i <= 10; $i++) {
            $w = 11-$i;
            $week_start = date('Y-m-d 00:00:00', strtotime('-'.(date('w')-1+($w*7)).' days'));
            $week_end = date('Y-m-d 00:00:00', strtotime('-'.(date('w')-1+($w*7)-7).' days'));
            $this->db->from('luxyart_tb_management_point');
            $this->db->where('status_change_point',2);
            $this->db->where('date_add >=', $week_start);
            $this->db->where('date_add <', $week_end);
            $this->db->select('SUM(point) as total_point');
            $result = $this->db->get()->row();
            array_push($use_point_week, $result->total_point==''?0:$result->total_point);
        }
        $data['use_point_week'] = "[".implode($use_point_week, ', ')."]";
        ...
        for($i = 1; $i <= 10; $i++) {
            ...
            $this->db->select('SUM(point) as total_point');
            $result = $this->db->get()->row();
            array_push($buy_point_week, $result->total_point==''?0:$result->total_point);
            array_push($list_week, 'Week '.$i.'<br>'.date('d/m', strtotime($week_start)).'-'.date('d/m', strtotime($week_end)));
        }
        $data['buy_point_week'] = "[".implode($buy_point_week, ', ')."]";

        $data['list_week'] = "[".implode(array_map(function($value){return "\"$value\"";},$list_week),', ')."]";
    ...
    public function get_list_user($search = false, $pagination = false, $limit = null, $begin = null){
        ...
        $listKey = ['user_email','user_firstname','user_lastname'];
        ...
        if($search) {
            ...
            $like = "(user_email LIKE '%$keyword%' OR ((".$like_user_firstname.") AND (".$like_user_firstname.")) OR user_firstname LIKE '%$keyword%' OR user_lastname LIKE '%$keyword%')";
    ...
    public function add_edit_user() {
        ...
        $avatar = $this->upload('avatar', 'avatar_'.time().'.png', 'publics/avatar');
        if($avatar) $item['user_avatar'] = $avatar; 

        $banner = $this->upload('banner', 'banner_'.time().'.png', 'publics/banner');
        if($banner) $item['user_banner'] = $banner; 

        $this->db->where('user_email', $item['user_email']);
        $this->db->where('user_id !=', $id);
        if(count($this->db->get('luxyart_tb_user')->result())>0) {
            $this->res(false, "Email exists !<br> Please choose email other !");
            return $this->res();
        }

    	if($id != null) {
            $get = $this->getData($id, 'luxyart_tb_user', 'user_id');
            if($avatar) $this->deleteFile('publics/avatar/'.$get->user_avatar);
            if($banner) $this->deleteFile('publics/banner/'.$get->user_banner);
            ...
            return $this->messageCreate($this->insertData("luxyart_tb_user", $item));  
    	}
        return $this->res();
    }
    ...
    public function page_view_order($id) {
        ...
        $data['title'] = "売れた画像: ". $data['get']->id_order_img;
    ...
    public function page_add_edit_meta($id) {
        ...
        if(count($data['get'])>0) {
            $data['title']  = "メタ情報管理: ". $data['get']->page_title_name;
            $data['type']   = 'edit';
        }else{
            $data['title']  = "メタ情報管理";
            $data['type']   = 'add';
    ...
    public function page_add_edit_email_tpl($id) {
        ...
        if(count($data['get'])>0) {
            $data['title']  = "Emailテンプレート管理: ". $data['get']->subject;
            $data['type']   = 'edit';
        }else{
            $data['title']  = "Emailテンプレート管理";
            $data['type']   = 'add';
    ...
    public function page_add_edit_notification($id) {
        ...
        if(count($data['get'])>0) {
            $data['title']  = "お知らせ管理: ". $data['get']->title;
            $data['type']   = 'edit';
        }else{
            $data['title']  = "お知らせ管理";
            $data['type']   = 'add';
    ...
    public function get_list_image($search = false, $pagination = false, $limit = null, $begin = null){
        ...
        $this->db->join('luxyart_tb_user as user','user.user_id=img.user_id','LEFT');
        $this->db->join('luxyart_tb_manager_server as server','server.id_server=img.id_server','LEFT');
        $this->db->join('luxyart_tb_img_category as icc','icc.id_img=img.id_img','LEFT');
    ...
    public function add_edit_country() {
        ...
        if(count($this->getData(trim($item['country_name']),'luxyart_tb_country','country_name'))>0) {
            $this->res(false, "Name country exists !");
            return $this->res();
        }
    ...
    public function page_add_edit_country($id) {
        ...
        if(count($data['get'])>0) {
            $data['title']  = "都道府県管理: ". $data['get']->country_name;
            $data['type']   = 'edit';
        }else{
            $data['title']  = "都道府県管理";
            $data['type']   = 'add';
    ...
    public function add_edit_state() {
        ...
        if(count($this->getData(trim($item['state_name']),'luxyart_tb_country_state','state_name'))>0) {
            $this->res(false, "Name state exists !");
            return $this->res();
        }
    ...
    public function page_add_edit_category($id) {
        ...
        if(count($data['get'])>0) {
            $data['title']  = "カテゴリ管理: ". $data['get']->cate_name;
            $data['type']   = 'edit';
        }else{
            $data['title']  = "カテゴリ管理";
            $data['type']   = 'add';
    ...
    public function page_add_edit_package($id) {
        ...
        if(count($data['get'])>0) {
            $data['title']      = "ポイントパック管理: ". $data['get']->point_credit ;
            $data['type']       = 'edit';
        }else{
            $data['title']      = "ポイントパック管理";
            $data['type']       = 'add';
    ...
    public function page_add_edit_competition($id) {
        ...
        if(count($data['get'])>0) {
            $data['title']      = "コンペ管理: ". $data['get']->title_com;
            $data['type']       = 'edit';
        }else{
            $data['title']      = "コンペ管理";
            $data['type']       = 'add';
    ...
    public function add_edit_competition() {
        ...
        if($total > ($user->user_paymoney_getpoint+$user->user_point)) {
            $note['note_point'] = 'User this not enough point !';
        }

        if($this->input->post('img_color_id')==null) {
            $note['note_color'] = 'Must choose minimum 1 color !';
        }

        if(count($note)>0) {
            $this->res(false, 'Error some thing !', $note);
            return $this->res();
        }
        ...
        $upload = $this->upload('icon_com','competition_icon_'.time().'.png', 'publics/competition_icon');
        if($upload) $item['icon_com'] = $upload; 

        $upload = $this->upload('photo_des_com','competition_img_'.time().'.png', 'publics/competition_img');
        if($upload) $item['photo_des_com'] = $upload; 
    ...
    public function page_add_edit_inbox($id) {
        ...
        if(count($data['get'])>0) {
            $data['title']  = "通知メッセージ: ". $data['get']->subject;
            $data['type']   = 'edit';
        }else{
            $data['title']  = "通知メッセージ";
            $data['type']   = 'add';
    ...
    public function page_list_c_agree($page) { 
        ...
        $data['title'] = "List competition time agree";
        $data['keyword'] = $search;
    ...
    public function page_add_edit_c_agree($id) {
        ...
        if(count($data['get'])>0) {
            $data['title']  = "Edit compettion time agree: ". $data['get']->name;
            $data['type']   = 'edit';
        }else{
            $data['title']  = "Create compettion time agree New";
            $data['type']   = 'add';
        }
    ...
    public function send_mail_return_money($id) {
        ...
        $dataSendEmail["name"]                  = $get->user_firstname.' '.$get->user_lastname;
        $dataSendEmail["key_1"]                 = $get->tranfer_point_require;
        $dataSendEmail["key_2"]                 = $get->money_user_get;
        if($get->tranfer_status==2){
            $dataSendEmail["key_3"]             = "Transferred";
        }else{ 
            $dataSendEmail["key_3"]             = "Requested";
        }
        $this->function_model->sendMailHTML($dataSendEmail);
    }
    ...
    public function status_return_money($id) {
        ...
        $item = ['tranfer_status'=>$this->input->post('status')];
    ...
    public function page_add_edit_menu($id) {
        ...
        if(count($data['get'])>0) {
            $data['title']  = "メニュー: ". $data['get']->menu_ja;
            $data['type']   = 'edit';
        }else{
            $data['title']  = "メニュー";
            $data['type']   = 'add';
    ...
    public function send_mail_user_active_user_order($id) {
        ...
        $dataSendEmail["key_2"]                 = $get->point_credit;
        $dataSendEmail["key_3"]                 = $get->user_paymoney_getpoint;
        $dataSendEmail["key_4"]                 = $get->payment_status==1?"Received":"Wait";
    ...
    public function page_add_edit_qa_category($id) {
        ...
        if(count($data['get'])>0) {
            $data['title']  = "Update QA category: ". $data['get']->title;
            $data['type']   = 'edit';
        }else{
            $data['title']  = "Create QA Category";
            $data['type']   = 'add';
        }
    ...
    public function page_add_edit_qa($id) {
        ...
        if(count($data['get'])>0) {
            $data['title']  = "Update QA: ". $data['get']->title;
            $data['type']   = 'edit';
        }else{
            $data['title']  = "Create QA";
            $data['type']   = 'add';
        }
