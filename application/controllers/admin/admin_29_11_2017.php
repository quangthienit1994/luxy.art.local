...
class Admin extends MY_Admin {
    ...
    public function load_statistic_month($begin, $end) {
        ...
        $data['list_category'] = "['Jan', 'Feb', 'Mar', 'Apr','May', 'Jun', 'Jul', 'Aug','Sep', 'Oct', 'Nov', 'Dec']";
        return $data;
    }
    ...
    public function load_statistic_week($begin, $end) {
        ...
        for($i = 1; $i <= 10; $i++) {
            ...
            $result = $this->db->get()->row();
            array_push($buy_point, $result->total_point==''?0:$result->total_point);
            array_push($list, date('d/m', strtotime($week_start)).'-'.date('d/m', strtotime($week_end)));
        }
        $data['buy_point'] = "[".implode($buy_point, ', ')."]";

        $data['list_category'] = "[".implode(array_map(function($value){return "\"$value\"";},$list),', ')."]";
        return $data;
    }

    public function load_statistic_day($begin, $end) {
        $use_point = [];
        echo date('Y-m-d', strtotime($begin));
        for($i = 1; $i <= 10; $i++) {
            $w = 11-$i;
            $week_start = date('Y-m-d 00:00:00', strtotime('-'.$w.' days'));
            $week_end = date('Y-m-d 00:00:00', strtotime('-'.($w-1).' days'));
            $this->db->from('luxyart_tb_management_point');
            $this->db->where('status_change_point',2);
            $this->db->where('date_add >=', $week_start);
            $this->db->where('date_add <', $week_end);
        ...
        for($i = 1; $i <= 10; $i++) {
            ...
            $result = $this->db->get()->row();
            array_push($buy_point, $result->total_point==''?0:$result->total_point);
            array_push($list, date('d/m/Y', strtotime($week_start)));
        }
        $data['buy_point'] = "[".implode($buy_point, ', ')."]";
