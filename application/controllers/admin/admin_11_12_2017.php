...
class Admin extends MY_Admin {
    ...
    public function messageUpdate($data) {
        if($data){
            $this->res(true, "アップデート成功 !");
        }else{
            $this->res(false, "アップデートエラー !");
        }
        return $this->res();
    }
    ...
    public function add_edit_user() {
        ...
        if(count($this->db->get('luxyart_tb_user')->result())>0) {
            $note['note_email'] = "Email exists ! Please choose email other !";
        }
    ...
    public function page_add_edit_lang($id) {
        ...
        $data['title'] = "言語を更新する: ".$nameLang[str_replace('application/language/', '', $data['path'])];
    ...
    public function add_edit_competition() {
        ...
        if($this->input->post('img_color_id')==null) {
            $note['note_color'] = '最小1色を選択する必要があります !';
        }
    ...
    public function send_mail_user_active_user_order($id) {
        ...
        $dataSendEmail["key_5"]                 = $get->payment_status==1?"Received":"Wait";
    ...
    public function page_add_edit_qa_category($id) {
        ...
        if(count($data['get'])>0) {
            $data['title']  = "カテゴリを更新する: ". $data['get']->title;
            $data['type']   = 'edit';
        }else{
            $data['title']  = "カテゴリを作成する";
            $data['type']   = 'add';
        }
    ...
    public function page_add_edit_qa($id) {
        ...
        if(count($data['get'])>0) {
            $data['title']  = "カテゴリを作成する QA: ". $data['get']->title;
            $data['type']   = 'edit';
        }else{
            $data['title']  = "カテゴリを作成する QA";
            $data['type']   = 'add';
        }
