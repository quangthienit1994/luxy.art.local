...
class Images extends CI_Controller {
	...
	function list_comment(){
		...
		if($this->input->post('searchCity',TRUE)){
				
			$data['searchCity']['city_name'] = $this->input->post('city_name',TRUE);
			$config['total_rows'] = count($this->manager_comment_model->list_search_comment('', '', $data)->result());
			if($data['searchCity']['city_name'] == ""){
				$config['base_url'] = base_url('admin/list_comment/search/no_key');
			}
			else{
				$config['base_url'] = base_url('admin/list_comment/search/'.$data['searchCity']['city_name']);
			}
			$config['per_page'] = 10;
			$config['uri_segment'] = 4;
			$this->pagination->initialize($config);
			$data['phantrang'] = $this->pagination->create_links();
			$data['list_comment'] = $this->manager_comment_model->list_search_comment($config['per_page'], $this->uri->segment(4), $data)->result();
		...
		elseif($this->uri->segment(3) == "search"){
			...
			if($this->uri->segment(4) != ""){
				...
				$data['searchCity']['city_name'] = $this->uri->segment(4);
				if($data['searchCity']['city_name'] == "no_key")
					$data['searchCity']['city_name'] = "";
				$config['total_rows'] = count($this->manager_comment_model->list_search_comment('', '', $data)->result());
				$config['base_url'] = base_url('admin/list_comment/search/'.$this->uri->segment(4));
				$config['per_page'] = 10;
				$config['uri_segment'] = 5;
				$this->pagination->initialize($config);
				$data['phantrang'] = $this->pagination->create_links();
				$data['list_comment'] = $this->manager_comment_model->list_search_comment($config['per_page'],$this->uri->segment(5), $data)->result();
			...
			$this->pagination->initialize($config);
			$data['list_comment'] = $this->manager_comment_model->list_search_comment($config['per_page'], $this->uri->segment(3), $data)->result();
		...
		$this->template->load('admin', 'list_comment', $data, __CLASS__);
	...
	function detail_comment($id_img){
		...
		$config['total_rows'] = count($this->manager_comment_model->get_manager_list_comment_by_level_paging($id_img,1,'','','')->result());
		...
		$config['base_url'] =  base_url().'/admin/detail_comment/'.$id_img;
	
		$data['list_comment'] = $this->manager_comment_model->get_manager_list_comment_by_level_paging($id_img,1,'',$config['per_page'],$this->uri->segment(4))->result();
		...
		$data['title'] = $this->lang->line('title_page_detail_comment');
		$data['title'] = str_replace("{img_title}", $detail_product->img_title, $data['title']);
	
		$this->template->load('admin', 'detail_comment', $data, __CLASS__);
