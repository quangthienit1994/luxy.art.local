...
class Images extends CI_Controller {
	...
	public function post_image_test(){
		...
		if(isset($_POST['btnPostImage'])){
			...
			if($this->function_model->check_md5_file($md5_file) == 0){
				...
				if($type_export_image == 1){
					...
					imagejpeg($img, $dir_end, 30);
						
					if($kind_img == 1 && $width > $data['min_width_img']){
						$this->function_model->resize_width($data['min_width_img'], DIR_TEMP.$image_blur_no_extension, DIR_TEMP.$image_blur);
					}
					elseif($kind_img == 2 && $height > $data['min_height_img']){
						//$this->function_model->resize_height($data['min_height_img'], DIR_TEMP.$image_blur_no_extension, DIR_TEMP.$image_blur);
						$this->function_model->resize_width($data['min_width_img'], DIR_TEMP.$image_blur_no_extension, DIR_TEMP.$image_blur);
					}
						
					$image = imagecreatefromjpeg($image_link."/".$image_blur);
					for ($x=1; $x<=75; $x++){
						imagefilter($image, IMG_FILTER_GAUSSIAN_BLUR);
					}
					imagefilter($image, IMG_FILTER_SMOOTH,99);
					imagefilter($image, IMG_FILTER_BRIGHTNESS, 10);
					imagejpeg($image, $image_upload."/".$image_blur);
					imagedestroy($image);
					$image_new = $image_blur;
					...
					elseif($kind_img == 2 && $height > $data['min_height_img_thumnail']){
						$this->function_model->resize_height($data['min_height_img_thumnail'], DIR_TEMP.$image_thumnail_no_extension, DIR_TEMP.$image_blur);
					}
					...
					elseif($kind_img == 2 && $height > $data['min_height_img']){
						$this->function_model->resize_height($data['min_height_img'], DIR_TEMP.$image_watermark_no_extension, DIR_TEMP.$image_watermark);
					}
						
					$this->function_model->watermark_image(DIR_TEMP.$image_name, DIR.'img/watermark.png', DIR_TEMP."/".$image_watermark, 0);
					$this->function_model->watermark_image(DIR_TEMP."/".$image_watermark, DIR.'img/watermark.png', DIR_TEMP."/".$image_watermark, 1);
					$this->function_model->watermark_image(DIR_TEMP."/".$image_watermark, DIR.'img/watermark.png', DIR_TEMP."/".$image_watermark, 2);
					$this->function_model->watermark_image(DIR_TEMP."/".$image_watermark, DIR.'img/watermark.png', DIR_TEMP."/".$image_watermark, 3);
					$this->function_model->watermark_image(DIR_TEMP."/".$image_watermark, DIR.'img/watermark.png', DIR_TEMP."/".$image_watermark, 4);
					...
					elseif($kind_img == 2 && $height > $data['min_height_img_thumnail']){
						$this->function_model->resize_height($data['min_height_img_thumnail'], DIR_TEMP.$image_thumnail_no_extension, DIR_TEMP.$image_watermark);
					}
				...
				for($i=0; $i<count($category_sub); $i++){
					...
					$sub_cate = $category_sub[$i];
						
					$this->db->from('luxyart_tb_product_category');
					$this->db->select('*');
					$this->db->where(array('product_category_id' => $sub_cate, 'status_cate' => 1, 'lang_id' => 1));
					$rsProductCategory = $this->db->get();
	
					if(!empty($rsProductCategory)){
						 
						$resultProductCategory = $rsProductCategory->row();
						$parent_cate = $resultProductCategory->parent_cate;
						$root_cate = $resultProductCategory->root_cate;
		...
		$this->template->load('admin', 'post_image_test', $data, __CLASS__);
    ...
    public function edit_image($id_img){
		...
		$data['list_product_tags_sub'] = $this->manager_image_model->get_product_tags_not_contain($data['detail_product']->img_tags, 1)->result();
