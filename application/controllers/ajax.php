...
class Ajax extends CI_Controller {
	...
	function autorun_compe_choice_image(){
		...
		$dataSendEmail["name"] 					= "Do Huu Thien";
		$dataSendEmail["num_day_extend"]		= 1;
		$dataSendEmail["title_com"]				= "Title com 1";
		$dataSendEmail["link"]					= base_url()."compe-detail/1";
		$dataSendEmail["date_start_com"]		= "2018-03-15";
		$dataSendEmail["date_end_com"]			= "2018-09-11";
		$dataSendEmail["point_img_com"]			= 5;
		$dataSendEmail["img_quantity_com"]		= 7;
		//$dataSendEmail["is_debug"]			= 1;
		...
		$dataSendEmail["name_email_code"] 		= "autorun_compe_choice_image_before_num_days";
		$dataSendEmail["lang_id"] 				= $this->lang_id;
		$dataSendEmail["email_to"] 				= "tranduyhuu187@gmail.com";
		$dataSendEmail["name"] 					= "Tran Duy Huu";
		$dataSendEmail["num_day_extend"]		= 1;
		$dataSendEmail["title_com"]				= "Title com 2";
		$dataSendEmail["link"]					= base_url()."compe-detail/2";
		$dataSendEmail["date_start_com"]		= "2018-07-35";
		$dataSendEmail["date_end_com"]			= "2018-11-05";
		$dataSendEmail["point_img_com"]			= 3;
		$dataSendEmail["img_quantity_com"]		= 4;
		//$dataSendEmail["is_debug"]			= 1;
	...
	function fixpoint(){
		...
		$this->db->select('*');
		$this->db->where('content like "%720%" and point = "720.00"');
	
		$rsManagementPoint = $this->db->get();
		$resultManagementPoint = $rsManagementPoint->result();
	
		$i=0;
	
		foreach($resultManagementPoint as $_resultManagementPoint){
	
			$management_point_id = $_resultManagementPoint->management_point_id;
			$content = $_resultManagementPoint->content;
			$point = $_resultManagementPoint->point;
			
			$content_new = str_replace("720 LUX", "72 LUX", $content);
			$point_new = str_replace("720.00", "72.00", $point);
			
			//echo "management_point_id = ".$management_point_id."<br>"; 
			//echo "content = ".$content."<br>";
			//echo "point = ".$point."<br>";
			//echo "content_new = ".$content_new."<br>";
			//echo "point_new = ".$point_new."<br>";
			
			$this->db->where('management_point_id', $management_point_id);
			$this->db->update('luxyart_tb_management_point',array('content' => $content_new,'point' => $point_new));
		...
		echo "count management point = ".$i."<br>";
		
		echo "------------------------------------<br>";
		
		$this->db->from('luxyart_tb_user_message');
		$this->db->select('*');
		$this->db->where('ms_content like "%720%"');
		...
		foreach($resultUserMessage as $_resultUserMessage){
			...
			$ms_content_new = str_replace("720 LUX", "72 LUX", $ms_content);
				
			//echo "id_message = ".$id_message."<br>";
			//echo "ms_content = ".$ms_content."<br>";
			//echo "ms_content_new = ".$ms_content_new."<br>";
			
			$this->db->where('id_message', $id_message);
			$this->db->update('luxyart_tb_user_message',array('ms_content' => $ms_content_new));
		...
		echo "count user message = ".$i."<br>";
		
	}
	
	function delete_file_temp(){
		
		$arr_dir = array("publics/product_img/zip/", "publics/product_img/download/", "publics/product_img/temp/");
		...
		foreach($arr_dir as $_dir){
			...
			foreach($images as $curimg){
				...
				if(!in_array($curimg, $ignore)){
					...
					$filemtime = filemtime($dirname.$curimg);
			
					if(date("Y-m-d",$filemtime) < date('Y-m-d', time())){
							
						echo "file = ".$curimg."<br>";
							
						if(file_exists($dirname.$curimg)){
							unlink($dirname.$curimg);
							echo "deleted it: file = ".$dirname.$curimg."<br>";
						}
