...
class SFTP {
    ...
    public function upload($locpath, $rempath, $mode = 'auto', $permissions = NULL)
    {
        ...
        $stream = @fopen("ssh2.sftp://".intval($this->sftp)."$rempath", 'w');
    ...
    public function download($rempath, $locpath)
    {
        ...
        $stream = @fopen("ssh2.sftp://".intval($this->sftp)."$rempath", 'r');
		try {
			 if (!$stream) 
			 	{
        		throw new Exception("Could not open remote file: $rempath");
    			}
		} catch (Exception $e) {
			//echo 'ssss';
	...
	function check_file($filename,$pathserver)
	{
		if ( ! $this->_is_conn())
		{
			return FALSE;
		...
		echo '<br/>';
		echo $filename;
		//$data = @fopen("ssh2.sftp://".intval($this->sftp)."$pathserver$filename", 'r');
		$data = file_get_contents("ssh2.sftp://".intval($this->sftp)."/$pathserver/$filename");
		try {
			 if (!$data) 
			 	{
        		throw new Exception("Could not open remote file: $rempath");
