...
<section class="content_box">
    <article class="member_info">
      ...
      <table class="">
          <tbody>
            <tr>
                ...
                <th colspan="3"><?php echo $this->lang->line('operation'); ?></th>
           	...
           	<tr>
                ...
                <td style="width:100px;">
                  <form style="padding-top: 2px;" action="admin/status_user/<?=$value->user_id?>" method="post" onsubmit="return submitAjax(this,{'load':1000,'confirm':true})">
                        ...
                        <a onclick="submitForm(this)" href="javascript:void(0)"><?php echo $this->lang->line('delete'); ?></a>
                  </form>
                </td>
              	<td style="width:100px;">
                  <?php 
                ...
                <!--<td>
                	<a onclick="delete_user(<?=$value->user_id?>);">削除</a>
                </td>-->
      ...
      <center>
        <ul class="pagination">
       		<?php
            	//echo get_pagination_links($currentPage,$numPage,base_url()."admin/page_list_user/{page}?keyword=".$keyword."&user_level=".$user_level);
			?>
        </ul>
      </center>
...
function get_pagination_links($current_page, $total_pages, $url){
    ...
    if ($total_pages >= 1 && $current_page <= $total_pages) {
        ...
        if ($i != $total_pages)
            $links .= " ... ";
        $links .= "<a href=\"{$url}\">{$total_pages}</a>";
    }
    return $links;
}
?>
...
function delete_user(user_id){
	...
	if(confirm("Are you sure delete this user?")){
		...
		$.ajax({
			type: "POST",cache: false,
			url: "<?php echo base_url(); ?>ajax/delete_user",
