...
function check_error_register(){
	...
	if(error ==0) {		
			...
			$.ajax({
					...
					data: "email="+re_email+"&username="+re_username+"&passord="+re_password+"&has_notice="+has_notice+"&member_id="+member_id+"&email_invite="+email_invite+"&invite_member="+invite_member,
					async:false,
					success: function(data) {
