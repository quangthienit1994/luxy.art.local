			...
			function display_email(t, dupEmailCheck) {
				...
				for	(var i = 0; i < arr.length; i++) {
					...
					if ( dupEmailCheck === true && $orig.val().indexOf(arr[i]) != -1 ) {
						...
						var email_check = arr[i];
						var url_website = location.protocol + "//" + location.host;
						$.ajax({
							type: "POST",cache: false,
							url: url_website + "/ajax/check_email_exist",
							...
							success: function(data) {
							  ...
							  .prepend($(deleteIconHTML)
								...
								if(data == 2){
									$("#str_email_msg").css('display','block');
									$("#str_email_msg").css('color','red');
									$("#str_email_msg").css('font-weight','bold');
								$("#str_email_msg").append('<br/>Lỗi : Tài khoản Nhà Tuyển Dụng : ' + arr[i]);
								}
							},
							async: false
						});
