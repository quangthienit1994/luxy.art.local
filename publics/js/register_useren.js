...
function check_error_register(){
	...
	if($('#re_email').val()==""){
				...
				//goi function kiem tra trung email bao loi
				 $.ajax({
					...
					success: function(data) {
					                    ...
					                    {
											$('#re_email_msg').css('display','block');
											 $('#re_email_msg').text("Email already used. Try another email");
											 set_focus("re_email");
											 error =1;
					                    }
					                    else if(msg[1]==0)
	 ...
	 if($("#re_password").val()==""){
		...
		$('#re_password_msg').text("Please input password"); 
		set_focus("re_password");
		error =1;
	}else{
	...
	if($("#re_againpassword").val()==""){
		...
		$('#re_againpassword_msg').text("Please input password again");	
		set_focus("re_againpassword");
		error =1;
	}else{
			if($("#re_password").val()!=$("#re_againpassword").val())
			{
				...
				$('#re_againpassword_msg').css('display','block');
				$('#re_againpassword_msg').text("Password confirmation do not match");	
				set_focus("re_againpassword");
				error =1;	
			}
			else
	...
	if($("#username").val()==""){
		...
		$('#re_username_msg').text("Please input display name"); 
		set_focus("re_password");
		error =1;
	}
	if(error ==0) {		
			...
			$.ajax({
					...
					success: function(data) {
										...
										if(msg[1]==1)
					                    {
											...
											$('#error_register').text("Error connect....Please try again later");
											return false;
					                    }
					                    else if(msg[1]==0)
					                    {
