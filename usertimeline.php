...
$slct_stmnt = 'SELECT * FROM luxyart_tb_sitemap WHERE sitemap_id in (21) and status = 1 ORDER BY FIELD(sitemap_id,21)';
    ...
    while($rowMain = $resultMain->fetch_assoc()) {
		...
		if($is_params == 0){
			...
			if($sitemap_id == 21){
				...
				$sql = "SELECT user_id, user_dateregister FROM luxyart_tb_user WHERE user_status = 1 and is_active = 1 ORDER BY user_id DESC";
				...
				if($result->num_rows > 0){
					...
					while($row = $result->fetch_assoc()){
						...
						$user_id = $row['user_id'];
						$user_dateregister = $row['user_dateregister'];

						$link1 = str_replace("{user_id}", $user_id, $link);
						...
						$id_lastmod = $xml->createElement("lastmod", date('Y-m-d', strtotime($user_dateregister)));
						$id_lastmod = $item->appendChild($id_lastmod);
...
$output = $xml->saveXML();
$xml->save("usertimeline.xml");
$con->close();

echo "Create usertimeline.xml ok!";
